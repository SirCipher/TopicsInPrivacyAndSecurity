public class Steganography2 extends Steganography_Abstract {
	...
	protected void insert_hidden_text(byte[] image, String text) {
		...
		if (4 * buffer.length > image.length) {
		...
		for (int i=0, offset=0; i<buffer.length; ++i) {
			...
			for (int j=3; j>=0; j--, offset++) {
				// Extract bit from secret text
				int b = (buffer[i] >>> (2*j)) & 3;
	...
	protected byte[] extract_hidden_text(byte[] image) {
		...
		for (int b=0, offset=16; b<result.length; b++) {
